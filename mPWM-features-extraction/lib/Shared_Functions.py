...
def Classification_Train_Test(names, classifiers, X_train, y_train, X_test, y_test ):
    print('\n-->  Run Classification Training/Testing Split : \n', names)
...
#        print('Train/Test Split using:',name)
        ...
        print('\nClf=',name,': FeatureSize=',X_train.shape[0],', accuracy=',accuracy, ' sensitivity=',sensitivity,' specificity=', specificity,' sensitivity=',sensitivity,
    ...
    print('\n    ==> Classification Done.\nThe optimal classifier is:',clf_op , ', Accuracy  :',acc_op,'\n Feature size =:',X_test.shape[1],'\n')


    return Mdl_score, acc_op, clf_name, clf_op



def balance_classes(y, N=0):
    classes=np.unique(y)
    idx=[]
    
    for C in classes:
        idx_c = list(np.where(y == C))
        idex= [item for t in idx_c for item in t] 
        idx=idx+idex[:N] 
